...
void DefaultDeflateDecoder::decode(const char *deflateStream) {
    ...
    do {
        //BFINAL
        lastBlock = getBitFromStream(deflateStream, bitIndex++);
        ...
        for (unsigned i = 0; i < BTYPE_SIZE; i++) {
            btype <<= 1u;
            btype &= getBitFromStream(deflateStream, bitIndex++);
        }
...
bool DefaultDeflateDecoder::getBitFromStream(const char *stream, size_t index) const {
